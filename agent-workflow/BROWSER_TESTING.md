...
8. Stop and prepare a recovery summary if the same browser/runtime error repeats, a fix creates unrelated failures, or the root cause cannot be verified.
9. Return concise manual retest steps, changed files, verification results, assumptions, risks, and blockers.
10. Update `HANDOFF.md` after meaningful work.
...
| Finding | Update |
|---|---|
| New bug, regression, failed check, unresolved limitation | `agent-workflow/HANDOFF.md`, Known Issues section |
| Repeated failure, lost context, or recovery stop | `agent-workflow/HANDOFF.md`, Dumb-Zone Recovery section |
...
- Refresh context from `HANDOFF.md`, changed files, and test results after several edits or failed verification rather than rereading unrelated workflow files.
